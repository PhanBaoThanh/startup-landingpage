...
const Header = () => {
    ...
    return (
        <>
            <div className={`header ${isScroll ? 'isScroll' : ''}`}>
                <div className='headerContainer'>
                    <a href="/" className='headerLogo'>
                        <img src={isScroll? logo2 : logo} alt='ptc' />
                    </a>

                    <div className='headerNav'>
                        {data.map((item,index) => (<a href={item.link} className={`headerNavItem ${isScroll ? 'isBlack' : ''}`} key={index}>{item.name}</a>))}
                    </div>

                    <div className='btns'>
                        <Btn isTransparent={true} text='Get Started' type={isScroll? 'isScroll' : ''}/>
                        <div className='btnSidebar' onClick={() => setIsOpen(true)}>
                            <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 512 512" height="26px" width="26px" xmlns="http://www.w3.org/2000/svg"><path d="M64 384h384v-42.666H64V384zm0-106.666h384v-42.667H64v42.667zM64 128v42.665h384V128H64z"></path></svg>
                        </div>
                    </div>
                </div>
            </div>
            <div className={`sidebarMobile ${isOpen ? 'isOpenModal' : ''}`}>
                <div className={`sidebarMobileBox ${isOpen ? 'isOpenModalBox' : ''}`}>
                    <div className='navMobile'>
                        {data.map((item,index) => (<a className='navMobileItem' key={index} href={item.link}>{item.name}</a>))}
                    </div>
                    <div className='logoIcons'>
                        <a href='/' className='logoIcon'><svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 320 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z"></path></svg></a>
                    ...
                    <button className='sidebarMobileCloseBtn' onClick={() => setIsOpen(false)}>X</button>
                </div>
            </div>
